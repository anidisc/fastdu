...
// Basic POSIX types missing in MSVC
typedef int mode_t;
typedef intptr_t ssize_t;
typedef int pid_t;

// POSIX file flags to 0 as placeholders
#define O_DIRECTORY 0
#define O_NOFOLLOW 0
#define O_CLOEXEC 0

// Standard file descriptors and isatty
#ifndef STDIN_FILENO
...
// Redefine stat structure and functions to _stat64
#define stat _stat64
...
// sysconf and getmouse mappings for Windows
#define _SC_NPROCESSORS_ONLN 1
static inline long sysconf(int name) {
    if (name == _SC_NPROCESSORS_ONLN) {
        SYSTEM_INFO sysinfo;
        GetSystemInfo(&sysinfo);
        return sysinfo.dwNumberOfProcessors > 0 ? (long)sysinfo.dwNumberOfProcessors : 1L;
    }
    return 1L;
}
#define getmouse(x) nc_getmouse(x)

// getline and localtime_r emulations for Windows
...
// POSIX dirent structure and directory traversal emulation for Windows (UTF-8 compatible)
struct dirent {
    char d_name[260];
...
static inline DIR *opendir(const char *name) {
    ...
    if (!dir) return NULL;
    wchar_t wname[MAX_PATH + 4];
    int len = MultiByteToWideChar(CP_UTF8, 0, name, -1, wname, MAX_PATH);
    if (len <= 0) { free(dir); return NULL; }
    if (len > 1 && (wname[len-2] == L'\\' || wname[len-2] == L'/')) {
        wname[len-2] = L'\0';
        len--;
    }
    wcscat(wname, L"\\*");
    dir->hFind = FindFirstFileW(wname, &dir->find_data);
...
static inline struct dirent *readdir(DIR *dir) {
    ...
    if (dir->first) {
        ...
        if (!FindNextFileW(dir->hFind, &dir->find_data)) {
            return NULL;
        }
    }
    WideCharToMultiByte(CP_UTF8, 0, dir->find_data.cFileName, -1, dir->ent.d_name, sizeof(dir->ent.d_name), NULL, NULL);
...
// Emulate clock_gettime for Windows
...
// Light pthread emulation using Win32 threads and CRITICAL_SECTIONs
...
static inline int pthread_cond_timedwait(pthread_cond_t *c, pthread_mutex_t *m, const struct timespec *abstime) {
    ...
    long long abs_ms = (long long)abstime->tv_sec * 1000 + abstime->tv_nsec / 1000000;
    long long diff = abs_ms - now_ms;
    DWORD timeout = (diff <= 0) ? 0 : (DWORD)diff;
    BOOL ok = SleepConditionVariableCS(c, m, timeout);
    if (!ok) {
...
static DWORD WINAPI win_thread_proc(LPVOID lpParam) {
    win_thread_args_t *args = (win_thread_args_t*)lpParam;
    pthread_start_routine fn = args->fn;
    void *arg = args->arg;
    free(args);
    fn(arg);
    return 0;
...
static inline int pthread_create(pthread_t *t, void *attr, pthread_start_routine start_routine, void *arg) {
    (void)attr;
    win_thread_args_t *args = malloc(sizeof(win_thread_args_t));
    if (!args) return -1;
    args->fn = start_routine;
    args->arg = arg;
    *t = CreateThread(NULL, 0, win_thread_proc, args, 0, NULL);
    return (*t == NULL) ? -1 : 0;
}
...
// POSIX regex emulation fallbacks using case-insensitive substring search
typedef struct {
    char pattern[256];
} regex_t;
#define REG_ICASE 1
...
static inline int regcomp(regex_t *preg, const char *regex, int cflags) {
    (void)cflags;
    strncpy(preg->pattern, regex, sizeof(preg->pattern) - 1);
    preg->pattern[sizeof(preg->pattern) - 1] = '\0';
    return 0;
}

static inline int regexec(const regex_t *preg, const char *string, size_t nmatch, void *pmatch, int eflags) {
    (void)nmatch; (void)pmatch; (void)eflags;
    return (win_strcasestr(string, preg->pattern) != NULL) ? 0 : 1;
}
...
static int cache_save(const char *root, const Cache *c) {
    ...
    if (!f) { free(buf); free(cache_path); return -1; }
    fwrite(buf, 1, off, f);
    fclose(f);
    unlink(cache_path);
 ...
 */
#ifdef _WIN32
static unsigned long long scan_dir_recursive_win32(const char *abs_path, const char *root, const char *cache_file_abs, Cache *cache, ScanUI *ui, InodeSet *is) {
    ...
    while ((de = readdir(dp)) != NULL) {
        ...
        struct stat stf;
        if (stat(child_abs, &stf) == 0) {
            if (S_ISDIR(stf.st_mode)) {
                unsigned long long sub = scan_dir_recursive_win32(child_abs, root, cache_file_abs, cache, ui, is);
                CacheEntry *e = cache_upsert(cache, root, child_abs, sub, time(NULL));
                if (e) {
                    e->ino = (unsigned long long)stf.st_ino;
                    e->dir_mtime = stf.st_mtime;
                }
                total += sub;
...
static unsigned long long scan_dir_recursive(const char *dir, const char *root, const char *cache_file_abs, Cache *cache, ScanUI *ui) {
    ...
    unsigned long long total = scan_dir_recursive_win32(dir, root, cache_file_abs, cache, ui, &is);
    inodeset_free(&is);
    return total;
}
#else
...
static int build_archive_view(DirView *out) {
    ...
    while (archive_read_next_header(a, &entry) == ARCHIVE_OK) {
        if (g_interrupted) { interrupted = 1; break; }
        
        // Non-blocking ESC check
        if (getch() == 27) { interrupted = 1; break; }

        draw_cache_progress(g_inside_archive_path, (FILE*)archive_read_userdata(a), (long)total_compressed_bytes, &spinner, &last_draw);

        const char *entry_path = archive_entry_pathname(entry);
        if (!entry_path) continue;

        // Strip leading slash if any
        if (entry_path[0] == '/') entry_path++;

        // We only care about entries that are direct children of out->path
        // out->path is e.g. "my_archive.zip/some_dir"
        // entry_path inside archive is "some_dir/file.txt"
        // Let's extract relative path of the entry from the subpath
        const char *sub = g_archive_subpath ? g_archive_subpath : "";
        size_t sublen = strlen(sub);
        
        if (sublen > 0) {
            if (strncmp(entry_path, sub, sublen) != 0) continue;
            // Check if there's a trailing slash in the match or if it's the start
            if (entry_path[sublen] != '/' && entry_path[sublen] != '\0') continue;
            entry_path += sublen;
            if (entry_path[0] == '/') entry_path++;
        }

        if (entry_path[0] == '\0') continue; // represents the directory itself

        // Now entry_path is relative to out->path, like "file.txt" or "subdir/other.txt"
        // We only want direct children, so no other slashes
        const char *slash = strchr(entry_path, '/');
        int is_dir = 0;
        char name[256];
        if (slash) {
            is_dir = 1;
            size_t len = (size_t)(slash - entry_path);
            if (len >= sizeof(name)) len = sizeof(name) - 1;
            memcpy(name, entry_path, len);
            name[len] = '\0';
        } else {
            // Might be a directory itself if it ends with / (though we stripped or handle it)
            // or if the mode says it's a directory
            if (S_ISDIR(archive_entry_mode(entry))) is_dir = 1;
            strncpy(name, entry_path, sizeof(name) - 1);
            name[sizeof(name) - 1] = '\0';
        }

        // De-duplicate: if we already added a directory entry with this name, skip
        int found = 0;
        for (size_t i = 0; i < out->n; i++) {
            if (strcmp(out->v[i].name, name) == 0) {
                // If it's a duplicate and the existing is a directory or this is a directory, keep directory
                if (is_dir) out->v[i].is_dir = 1;
                // Add size to existing folder
                if (is_dir) out->v[i].size += (unsigned long long)archive_entry_size(entry);
                found = 1;
                break;
            }
        }
...
#ifdef _WIN32
static unsigned long long count_dir_files_win32(const char *path) {
    ...
    while ((de = readdir(dp)) != NULL) {
        ...
        char *child_abs = path_join(path, de->d_name);
        if (!child_abs) continue;
        struct stat st0;
        if (stat(child_abs, &st0) == 0) {
            if (S_ISREG(st0.st_mode)) count++;
            else if (S_ISDIR(st0.st_mode)) {
                count += count_dir_files_win32(child_abs);
            }
        }
        free(child_abs);
    }
...
static unsigned long long sum_dir_sizes_win32(const char *path, InodeSet *is) {
    unsigned long long total = 0ULL;
    DIR *dp = opendir(path);
    if (!dp) return 0ULL;
    struct dirent *de;
    while ((de = readdir(dp)) != NULL) {
        ...
        if (stat(child_abs, &st) == 0) {
            if (S_ISREG(st.st_mode)) {
                ...
                total += sum_dir_sizes_win32(child_abs, is);
            }
        }
        free(child_abs);
    }
...
static unsigned long long sum_path_size(const char *path) {
    ...
    if (S_ISDIR(st.st_mode)) {
        unsigned long long s = sum_dir_sizes_win32(path, &is);
        inodeset_free(&is);
        return s;
    }
    inodeset_free(&is);
...
static unsigned long long count_files_path(const char *path) {
    ...
    if (S_ISDIR(st.st_mode)) {
        return count_dir_files_win32(path);
    }
    return 0ULL;
}
#else
...
#ifdef _WIN32
    (void)archive_path; (void)target_dir; (void)root; (void)cache;
    return -1;
#else
    struct stat st_arc;
...
static int zip_compress_items(char **src_paths, size_t num_src, const char *dest_zip, const char *root) {
#ifdef _WIN32
    (void)src_paths; (void)num_src; (void)dest_zip; (void)root;
    return -1;
#else
    ...
    for (size_t i = 0; i < n; i++) {
        ...
        if (S_ISDIR(st.st_mode)) {
            ...
            DIR *dp = opendir(list[i].abs);
            if (dp) {
                struct dirent *de;
                while ((de = readdir(dp)) != NULL) {
                    if (is_dot_or_dotdot(de->d_name)) continue;
                    char *c_abs = path_join(list[i].abs, de->d_name);
                    char *c_rel = path_join(list[i].rel, de->d_name);
                    if (c_abs && c_rel) {
                        if (n == cap) {
                            cap *= 2;
                            ZipTask *nl = realloc(list, cap * sizeof(ZipTask));
                            if (nl) list = nl;
                            else break;
                        }
                        list[n++] = (ZipTask){c_abs, c_rel};
                    } else {
                        free(c_abs); free(c_rel);
                    }
                }
                closedir(dp);
            }
            ...
            if (archive_write_header(a, entry) == ARCHIVE_OK) {
                ...
                if (fd >= 0) {
                    ...
                    while ((len = read(fd, buf, sizeof(buf))) > 0) {
                        ...
                        processed_bytes += (unsigned long long)len;
                        ...
                        if (ms > 50) {
                            last_draw = now;
                            int cols, rows; getmaxyx(stdscr, rows, cols);
                            double frac = (double)processed_bytes / (double)total_uncompressed_bytes;
                            if (frac > 1.0) frac = 1.0;
                            int barlen = cols > 40 ? cols - 40 : 20;
                            char bar[256]; int filled = (int)(frac * barlen);
...
static void format_owner_perm(const char *path, char *out, size_t outsz) {
    ...
    snprintf(out, outsz, "admin %04o", mode_octal);
#else
...
#ifdef _WIN32
static int collect_files_recursive_win32(const char *abs_path, FileCandidateList *list, int *interrupted_out) {
    DIR *dp = opendir(abs_path);
    if (!dp) return 0;
    ...
    while ((de = readdir(dp)) != NULL) {
        ...
        if (stat(child_abs, &st) == 0) {
            if (S_ISDIR(st.st_mode)) {
                collect_files_recursive_win32(child_abs, list, interrupted_out);
            } else if (S_ISREG(st.st_mode)) {
                if (st.st_size > 0) {
                    if (list->n == list->cap) {
                    ...
                    fc->path = child_abs; // transferred ownership
                    fc->size = (unsigned long long)st.st_size;
                    fc->head_hash = 0;
                    fc->dupe_group_id = 0;
                    child_abs = NULL; // ownership transferred
                }
            }
        }
        free(child_abs);
...
#ifdef _WIN32
    collect_files_recursive_win32(scan_root, &list, &interrupted);
#else
...
static void open_with_default(const char *path) {
    ...
    int len = MultiByteToWideChar(CP_UTF8, 0, path, -1, wpath, PATH_MAX);
    if (len > 0) {
        HINSTANCE res = ShellExecuteW(NULL, L"open", wpath, NULL, NULL, SW_SHOWNORMAL);
        if ((intptr_t)res <= 32) {
            snprintf(msg, sizeof(msg), "Failed to launch default app for: %s", path_basename_const(path));
            draw_status(msg);
        } else {
            snprintf(msg, sizeof(msg), "Opening: %s", path_basename_const(path));
            draw_status(msg);
        }
    } else {
        draw_status("Path conversion failed");
    }
#else
...
#ifdef _WIN32
static void enqueue_child_win32(DirTask *parent, const char *name) {
...
static void finalize_task_win32(DirTask *t) {
...
static void process_task(DirTask *t) {
    DIR *dp = opendir(t->abs_path);
    if (!dp) {
        if (atomic_fetch_sub(&t->pending, 1) == 1) {
            if (!tq_push_timed(t->finalq, t, 2000)) finalize_task_win32(t);
        }
        return;
    }
    ...
    while ((de = readdir(dp)) != NULL) {
        ...
        char *child_abs = path_join(t->abs_path, de->d_name);
        if (!child_abs) continue;
        ...
        if (stat(child_abs, &stf) == 0) {
            if (S_ISREG(stf.st_mode)) {
                if (stf.st_nlink <= 1 || !inodeset_check_and_add(t->is, stf.st_dev, stf.st_ino)) {
                    ...
                    cache_upsert_with_meta(t->cache, t->root, child_abs, b, time(NULL), (unsigned long long)stf.st_ino, 0);
                }
            } else if (S_ISDIR(stf.st_mode)) {
                enqueue_child_win32(t, de->d_name);
            }
        }
        free(child_abs);
    }
...
#ifdef _WIN32
static void *worker_loop_win32(void *arg) {
...
static void *finalizer_loop_win32(void *arg) {
    ScanPool *p = (ScanPool*)arg;
    for (;;) {
...
static unsigned long long scan_dir_parallel_deep(const char *root, const char *cache_abs, Cache *cache, int threads) {
    ...
    wg_init(&p.wg);
    for (int i = 0; i < threads; i++) pthread_create(&p.th[i], NULL, worker_loop_win32, &p);
    pthread_create(&p.fin_th, NULL, finalizer_loop_win32, &p);
...
#ifdef _WIN32
    if (system("where.exe bat >nul 2>nul") == 0) g_bat_cmd = "bat";
#else
...
#ifdef _WIN32
        fwrite(msg0, 1, sizeof(msg0)-1, stderr);
#else
